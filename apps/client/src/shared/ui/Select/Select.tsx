...
import type { SelectModel, Option, RenderOption, SelectInfinityProps } from './models';
    ...
    >
      ...
      {renderCustomMenu ? (
        renderCustomMenu({
          ...
          setHeaderText: setPreselectedHeaderText,
...
export type FormSelect = SelectModel & {
  name: string;
  errorName?: string;
};

export function FormSelect({ name, selectedOptionId, isRequired, errorName, ...props }: FormSelect) {
  const {
    control,
    setValue,
    formState: { errors },
  ...
  useEffect(() => {
    if (selectedOptionId) {
      setValue(name, selectedOptionId);
    }
  }, [selectedOptionId]);
  ...
  return (
    <Controller
      ...
      render={({ field: { value, onChange } }) => (
        <Select
          onChange={onChange}
          error={getInputError(errors, errorName ?? name)}
          {...props}
...
Select.InfinityList = InfinityList;
